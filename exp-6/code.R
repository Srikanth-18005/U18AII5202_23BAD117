...
ggparcoord(
  ...
  labs(title = "Parallel Coordinate Plot (Roll No: 23BAD117)") +
  theme_minimal()

ggplot(data, aes(x = Sales, y = Profit, size = Discount, color = Customer_Segment)) +
  geom_point(alpha = 0.6) +
  labs(
    title = "Bubble Chart: Sales vs Profit (Roll No: 23BAD117)",
    x = "Sales",
    y = "Profit"
  ) +
  theme_minimal()
...
xyplot(
  ...
  type = c("p", "r"),
  layout = c(3, 2),
  main = "Trellis Display by Region (Roll No: 23BAD117)",
  xlab = "Sales",
  ylab = "Profit"
)
